...
export default function AdminKurikulumPage() {
  ...
  return (
    <div className="space-y-6">
      ...
      <Dialog open={isMateriFormOpen} onOpenChange={(open) => { setIsMateriFormOpen(open); if (!open) setCurrentEditingMateri(null); }}>
        <DialogContent className="sm:max-w-lg h-[90vh] flex flex-col">
          <DialogHeader className="flex-shrink-0">
            <DialogTitle>{currentEditingMateri ? "Edit Materi" : "Tambah Materi Baru"}</DialogTitle>
          </DialogHeader>
          <Form {...materiAjarForm}>
            <form onSubmit={materiAjarForm.handleSubmit(handleMateriSubmit)} className="flex-grow flex flex-col overflow-y-hidden">
      ...
      <Dialog open={isSKLDialogOpen} onOpenChange={setIsSKLDialogOpen}><DialogContent className="sm:max-w-2xl max-h-[90vh] flex flex-col"><DialogHeader className="flex-shrink-0"><DialogTitle>Manajemen SKL</DialogTitle><DialogDescription>Kelola Standar Kompetensi Lulusan.</DialogDescription></DialogHeader><div className="flex-grow overflow-y-hidden py-4 flex flex-col gap-4"><Button onClick={() => openSKLForm()} className="self-start"><PlusCircle className="mr-2 h-4 w-4" /> Tambah SKL</Button><ScrollArea className="flex-grow">{isLoadingSkl ? (<div className="flex justify-center items-center h-40"><Loader2 className="h-8 w-8 animate-spin text-primary" /></div>) : sklList.length > 0 ? (<Table><TableHeader className="bg-muted/50 sticky top-0"><TableRow><TableHead>Kode</TableHead><TableHead>Deskripsi</TableHead><TableHead>Kategori</TableHead><TableHead className="text-right">Tindakan</TableHead></TableRow></TableHeader><TableBody>{sklList.map(skl => (<TableRow key={skl.id}><TableCell className="font-medium">{skl.kode}</TableCell><TableCell className="max-w-md whitespace-pre-wrap">{skl.deskripsi}</TableCell><TableCell>{skl.kategori}</TableCell><TableCell className="text-right"><Button variant="ghost" size="sm" onClick={() => openSKLForm(skl)} className="mr-1"><Edit className="h-4 w-4" /></Button><Button variant="ghost" size="sm" onClick={() => openDeleteSKLDialog(skl)} className="text-destructive"><Trash2 className="h-4 w-4" /></Button></TableCell></TableRow>))}</TableBody></Table>) : (<p className="text-muted-foreground text-center pt-8">Belum ada SKL.</p>)}</ScrollArea></div><DialogFooter className="flex-shrink-0 border-t pt-4"><Button variant="outline" onClick={() => setIsSKLDialogOpen(false)}>Tutup</Button></DialogFooter></DialogContent></Dialog>
      ...
      <Dialog open={isCPDialogOpen} onOpenChange={setIsCPDialogOpen}><DialogContent className="sm:max-w-2xl max-h-[90vh] flex flex-col"><DialogHeader className="flex-shrink-0"><DialogTitle>Manajemen CP</DialogTitle><DialogDescription>Kelola Capaian Pembelajaran.</DialogDescription></DialogHeader><div className="flex-grow overflow-y-hidden py-4 flex flex-col gap-4"><Button onClick={() => openCPForm()} className="self-start"><PlusCircle className="mr-2 h-4 w-4" /> Tambah CP</Button><ScrollArea className="flex-grow">{isLoadingCp ? (<div className="flex justify-center items-center h-40"><Loader2 className="h-8 w-8 animate-spin text-primary" /></div>) : cpList.length > 0 ? (<Table><TableHeader className="bg-muted/50 sticky top-0"><TableRow><TableHead>Kode</TableHead><TableHead>Deskripsi</TableHead><TableHead>Fase</TableHead><TableHead>Elemen</TableHead><TableHead className="text-right">Tindakan</TableHead></TableRow></TableHeader><TableBody>{cpList.map(cp => (<TableRow key={cp.id}><TableCell className="font-medium">{cp.kode}</TableCell><TableCell className="max-w-md whitespace-pre-wrap">{cp.deskripsi}</TableCell><TableCell>{cp.fase}</TableCell><TableCell>{cp.elemen}</TableCell><TableCell className="text-right"><Button variant="ghost" size="sm" onClick={() => openCPForm(cp)} className="mr-1"><Edit className="h-4 w-4" /></Button><Button variant="ghost" size="sm" onClick={() => openDeleteCPDialog(cp)} className="text-destructive"><Trash2 className="h-4 w-4" /></Button></TableCell></TableRow>))}</TableBody></Table>) : (<p className="text-muted-foreground text-center pt-8">Belum ada CP.</p>)}</ScrollArea></div><DialogFooter className="flex-shrink-0 border-t pt-4"><Button variant="outline" onClick={() => setIsCPDialogOpen(false)}>Tutup</Button></DialogFooter></DialogContent></Dialog>
      ...
      <Dialog open={isKategoriMateriDialogOpen} onOpenChange={setIsKategoriMateriDialogOpen}><DialogContent className="sm:max-w-md max-h-[90vh] flex flex-col"><DialogHeader className="flex-shrink-0"><DialogTitle>Kelola Kategori Materi</DialogTitle><DialogDescription>Tambah atau lihat kategori untuk bank materi.</DialogDescription></DialogHeader><div className="flex-grow overflow-y-hidden py-4 flex flex-col gap-4"><Form {...kategoriMateriForm}><form onSubmit={kategoriMateriForm.handleSubmit(handleKategoriMateriSubmit)} className="flex gap-2 items-start"><FormField control={kategoriMateriForm.control} name="nama" render={({ field }) => (<FormItem className="flex-grow"><FormLabel className="sr-only">Nama Kategori</FormLabel><FormControl><Input placeholder="Nama kategori baru" {...field} /></FormControl><FormMessage /></FormItem>)} /><Button type="submit" disabled={isKategoriMateriSubmitting}>{isKategoriMateriSubmitting ? <Loader2 className="h-4 w-4 animate-spin" /> : <PlusCircle className="h-4 w-4"/>}</Button></form></Form><ScrollArea className="flex-grow -mx-4 px-4">{isLoadingKategoriMateri ? (<div className="flex justify-center items-center h-40"><Loader2 className="h-8 w-8 animate-spin text-primary" /></div>) : kategoriMateriList.length > 0 ? (<Table><TableHeader><TableRow><TableHead>Nama Kategori</TableHead><TableHead className="text-right">Aksi</TableHead></TableRow></TableHeader><TableBody>{kategoriMateriList.map(kat => (<TableRow key={kat.id}><TableCell className="font-medium">{kat.nama}</TableCell><TableCell className="text-right"><Button variant="ghost" size="xs" className="text-destructive" onClick={() => openDeleteKategoriMateriDialog(kat)}><Trash2 className="h-3 w-3"/></Button></TableCell></TableRow>))}</TableBody></Table>) : (<p className="text-sm text-muted-foreground text-center pt-8">Belum ada kategori.</p>)}</ScrollArea></div><DialogFooter className="flex-shrink-0 border-t pt-4"><Button variant="outline" onClick={() => setIsKategoriMateriDialogOpen(false)}>Tutup</Button></DialogFooter></DialogContent></Dialog>
      ...
      <Dialog open={isSilabusDialogOpen} onOpenChange={setIsSilabusDialogOpen}><DialogContent className="sm:max-w-3xl max-h-[90vh] flex flex-col"><DialogHeader className="flex-shrink-0"><DialogTitle>Pengembangan Silabus</DialogTitle><DialogDescription>Kelola daftar silabus per mata pelajaran.</DialogDescription></DialogHeader><div className="flex-grow overflow-y-hidden py-4 flex flex-col gap-4"><Button onClick={() => openSilabusForm()} className="self-start"><PlusCircle className="mr-2 h-4 w-4" /> Tambah Silabus</Button><ScrollArea className="flex-grow">{isLoadingSilabus ? (<div className="flex justify-center items-center h-40"><Loader2 className="h-8 w-8 animate-spin text-primary" /></div>) : silabusList.length > 0 ? (<Table><TableHeader className="bg-muted/50 sticky top-0"><TableRow><TableHead>Judul</TableHead><TableHead>Mapel</TableHead><TableHead>Kelas</TableHead><TableHead>File</TableHead><TableHead className="text-right">Tindakan</TableHead></TableRow></TableHeader><TableBody>{silabusList.map(s => (<TableRow key={s.id}><TableCell className="font-medium max-w-xs truncate">{s.judul}</TableCell><TableCell>{s.mapel?.nama || "-"}</TableCell><TableCell>{s.kelas}</TableCell><TableCell>{s.namaFileOriginal || "-"}</TableCell><TableCell className="text-right"><Button variant="ghost" size="xs" onClick={() => openSilabusForm(s)} className="mr-1"><Edit className="h-3 w-3"/></Button><Button variant="ghost" size="xs" onClick={() => openDeleteSilabusDialog(s)} className="text-destructive"><Trash2 className="h-3 w-3"/></Button></TableCell></TableRow>))}</TableBody></Table>) : (<p className="text-center text-muted-foreground pt-8">Belum ada silabus.</p>)}</ScrollArea></div><DialogFooter className="flex-shrink-0 border-t pt-4"><Button variant="outline" onClick={() => setIsSilabusDialogOpen(false)}>Tutup</Button></DialogFooter></DialogContent></Dialog>
      <Dialog open={isSilabusFormOpen} onOpenChange={(open) => { setIsSilabusFormOpen(open); if(!open) setCurrentEditingSilabus(null);}}>
        <DialogContent className="sm:max-w-lg h-[90vh] flex flex-col">
          <DialogHeader className="flex-shrink-0">
            <DialogTitle>{currentEditingSilabus ? "Edit Silabus" : "Tambah Silabus"}</DialogTitle>
          </DialogHeader>
          <Form {...silabusForm}>
            <form onSubmit={silabusForm.handleSubmit(handleSilabusSubmit)} className="flex-grow flex flex-col overflow-y-hidden">
              <ScrollArea className="flex-grow px-6 -mx-6">
                <div className="py-4 space-y-4">
                  <FormField control={silabusForm.control} name="judul" render={({ field }) => (<FormItem><FormLabel>Judul Silabus</FormLabel><FormControl><Input placeholder="Silabus Matematika Semester Ganjil" {...field} /></FormControl><FormMessage /></FormItem>)} />
                  <FormField control={silabusForm.control} name="mapelId" render={({ field }) => (<FormItem><FormLabel>Mata Pelajaran</FormLabel><Select onValueChange={field.onChange} value={field.value}><FormControl><SelectTrigger><SelectValue placeholder="Pilih Mapel" /></SelectTrigger></FormControl><SelectContent>{mataPelajaranOptions.length > 0 ? mataPelajaranOptions.map(s => <SelectItem key={s.id} value={s.id}>{s.nama} ({s.kode})</SelectItem>) : <SelectItem value="" disabled>Tidak ada mapel</SelectItem>}</SelectContent></Select><FormMessage /></FormItem>)} />
                  <FormField control={silabusForm.control} name="kelas" render={({ field }) => (<FormItem><FormLabel>Kelas</FormLabel><FormControl><Input placeholder="Contoh: X IPA 1" {...field} /></FormControl><FormMessage /></FormItem>)} />
                  <FormField control={silabusForm.control} name="deskripsiSingkat" render={({ field }) => (<FormItem><FormLabel>Deskripsi (Opsional)</FormLabel><FormControl><Textarea placeholder="Ringkasan isi silabus..." {...field} value={field.value || ""} /></FormControl><FormMessage /></FormItem>)} />
                  <FormField control={silabusForm.control} name="file" render={({ field: { onChange, value, ...restField } }) => (<FormItem><FormLabel>File Silabus {currentEditingSilabus?.namaFileOriginal ? `(Kosongkan jika tidak ubah: ${currentEditingSilabus.namaFileOriginal})` : ""}</FormLabel><FormControl><Input type="file" onChange={(e) => onChange(e.target.files ? e.target.files[0] : null)} {...restField} /></FormControl><FormDescription>Unggah file PDF/DOCX.</FormDescription><FormMessage /></FormItem>)} />
                </div>
              </ScrollArea>
              <DialogFooter className="flex-shrink-0 pt-4 border-t">
                <Button type="button" variant="outline" onClick={() => {setIsSilabusFormOpen(false);setCurrentEditingSilabus(null);}} disabled={isSilabusSubmitting}>Batal</Button>
                <Button type="submit" disabled={isSilabusSubmitting || mataPelajaranOptions.length === 0}>{isSilabusSubmitting && <Loader2 className="mr-2 h-4 w-4 animate-spin" />}{currentEditingSilabus ? "Simpan" : "Tambah"}</Button>
              </DialogFooter>
            </form>
          </Form>
        </DialogContent>
      ...
      <Dialog open={isRPPDialogOpen} onOpenChange={setIsRPPDialogOpen}><DialogContent className="sm:max-w-3xl max-h-[90vh] flex flex-col"><DialogHeader className="flex-shrink-0"><DialogTitle>Penyusunan RPP</DialogTitle><DialogDescription>Kelola Rencana Pelaksanaan Pembelajaran.</DialogDescription></DialogHeader><div className="flex-grow overflow-y-hidden py-4 flex flex-col gap-4"><Button onClick={() => openRPPForm()} className="self-start"><PlusCircle className="mr-2 h-4 w-4" /> Tambah RPP</Button><ScrollArea className="flex-grow">{isLoadingRpp ? (<div className="flex justify-center items-center h-40"><Loader2 className="h-8 w-8 animate-spin text-primary" /></div>) :rppList.length > 0 ? (<Table><TableHeader className="bg-muted/50 sticky top-0"><TableRow><TableHead>Judul</TableHead><TableHead>Mapel</TableHead><TableHead>Kelas</TableHead><TableHead className="text-center">Pertemuan</TableHead><TableHead className="text-right">Tindakan</TableHead></TableRow></TableHeader><TableBody>{rppList.map(r => (<TableRow key={r.id}><TableCell className="font-medium max-w-xs truncate">{r.judul}</TableCell><TableCell>{r.mapel?.nama || "-"}</TableCell><TableCell>{r.kelas}</TableCell><TableCell className="text-center">{r.pertemuanKe}</TableCell><TableCell className="text-right"><Button variant="ghost" size="xs" onClick={() => openRPPForm(r)} className="mr-1"><Edit className="h-3 w-3"/></Button><Button variant="ghost" size="xs" onClick={() => openDeleteRPPDialog(r)} className="text-destructive"><Trash2 className="h-3 w-3"/></Button></TableCell></TableRow>))}</TableBody></Table>) : (<p className="text-center text-muted-foreground pt-8">Belum ada RPP.</p>)}</ScrollArea></div><DialogFooter className="flex-shrink-0 border-t pt-4"><Button variant="outline" onClick={() => setIsRPPDialogOpen(false)}>Tutup</Button></DialogFooter></DialogContent></Dialog>
      <Dialog open={isRPPFormOpen} onOpenChange={(open) => { setIsRPPFormOpen(open); if(!open) setCurrentEditingRPP(null);}}>
        <DialogContent className="sm:max-w-lg h-[90vh] flex flex-col">
          <DialogHeader className="flex-shrink-0">
            <DialogTitle>{currentEditingRPP ? "Edit RPP" : "Tambah RPP"}</DialogTitle>
          </DialogHeader>
          <Form {...rppForm}>
            <form onSubmit={rppForm.handleSubmit(handleRPPSubmit)} className="flex-grow flex flex-col overflow-y-hidden">
              <ScrollArea className="flex-grow px-6 -mx-6">
                <div className="py-4 space-y-4">
                  <FormField control={rppForm.control} name="judul" render={({ field }) => (<FormItem><FormLabel>Judul RPP</FormLabel><FormControl><Input placeholder="RPP Fungsi Kuadrat Pertemuan 1" {...field} /></FormControl><FormMessage /></FormItem>)} />
                  <div className="grid grid-cols-2 gap-4">
                    <FormField control={rppForm.control} name="mapelId" render={({ field }) => (<FormItem><FormLabel>Mata Pelajaran</FormLabel><Select onValueChange={field.onChange} value={field.value}><FormControl><SelectTrigger><SelectValue placeholder="Pilih Mapel" /></SelectTrigger></FormControl><SelectContent>{mataPelajaranOptions.length > 0 ? mataPelajaranOptions.map(s => <SelectItem key={s.id} value={s.id}>{s.nama} ({s.kode})</SelectItem>) : <SelectItem value="" disabled>Tidak ada mapel</SelectItem>}</SelectContent></Select><FormMessage /></FormItem>)} />
                    <FormField control={rppForm.control} name="kelas" render={({ field }) => (<FormItem><FormLabel>Kelas</FormLabel><FormControl><Input placeholder="X IPA 1" {...field} /></FormControl><FormMessage /></FormItem>)} />
                  </div>
                  <FormField control={rppForm.control} name="pertemuanKe" render={({ field }) => (<FormItem><FormLabel>Pertemuan Ke-</FormLabel><FormControl><Input type="number" placeholder="1" {...field} /></FormControl><FormMessage /></FormItem>)} />
                  <FormField control={rppForm.control} name="materiPokok" render={({ field }) => (<FormItem><FormLabel>Materi Pokok (Opsional)</FormLabel><FormControl><Textarea placeholder="Materi utama..." {...field} rows={2} value={field.value || ""}/></FormControl><FormMessage /></FormItem>)} />
                  <FormField control={rppForm.control} name="kegiatanPembelajaran" render={({ field }) => (<FormItem><FormLabel>Kegiatan Pembelajaran (Opsional)</FormLabel><FormControl><Textarea placeholder="Langkah-langkah kegiatan..." {...field} rows={3} value={field.value || ""}/></FormControl><FormMessage /></FormItem>)} />
                  <FormField control={rppForm.control} name="penilaian" render={({ field }) => (<FormItem><FormLabel>Penilaian (Opsional)</FormLabel><FormControl><Textarea placeholder="Teknik penilaian..." {...field} rows={2} value={field.value || ""}/></FormControl><FormMessage /></FormItem>)} />
                  <FormField control={rppForm.control} name="file" render={({ field: { onChange, value, ...restField } }) => (<FormItem><FormLabel>File RPP {currentEditingRPP?.namaFileOriginal ? `(Kosongkan jika tidak diubah)` : ""}</FormLabel><FormControl><Input type="file" onChange={(e) => onChange(e.target.files ? e.target.files[0] : null)} {...restField} /></FormControl><FormDescription>Unggah file PDF/DOCX.</FormDescription><FormMessage /></FormItem>)} />
                </div>
              </ScrollArea>
              <DialogFooter className="flex-shrink-0 pt-4 border-t">
                <Button type="button" variant="outline" onClick={() => {setIsRPPFormOpen(false);setCurrentEditingRPP(null);}} disabled={isRPPSubmitting}>Batal</Button>
                <Button type="submit" disabled={isRPPSubmitting || mataPelajaranOptions.length === 0}>{isRPPSubmitting && <Loader2 className="mr-2 h-4 w-4 animate-spin" />}{currentEditingRPP ? "Simpan" : "Tambah"}</Button>
              </DialogFooter>
            </form>
          </Form>
        </DialogContent>
