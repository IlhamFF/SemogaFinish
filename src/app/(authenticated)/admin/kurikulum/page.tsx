...
import { BookOpenCheck, Target, BookCopy, BookUp, Layers, FileText, FolderKanban, PlusCircle, Edit, Loader2, UploadCloud, Link2Icon, Trash2, Book, Library, List } from "lucide-react";
...
export default function AdminKurikulumPage() {
  ...
  const fetchSklData = async () => { setIsLoadingSkl(true); try { const response = await fetch('/api/kurikulum/skl'); if (!response.ok) throw new Error('Gagal mengambil data SKL'); setSklList(await response.json()); } catch (error: any) { toast({ title: "Error SKL", description: error.message || "Tidak dapat memuat SKL.", variant: "destructive" }); } finally { setIsLoadingSkl(false); } };
  const fetchCpData = async () => { setIsLoadingCp(true); try { const response = await fetch('/api/kurikulum/cp'); if (!response.ok) throw new Error('Gagal mengambil data CP'); setCpList(await response.json()); } catch (error: any) { toast({ title: "Error CP", description: error.message || "Tidak dapat memuat CP.", variant: "destructive" }); } finally { setIsLoadingCp(false); } };
  const fetchKategoriMateriData = async () => { setIsLoadingKategoriMateri(true); try { const response = await fetch('/api/kurikulum/materi-kategori'); if (!response.ok) throw new Error('Gagal mengambil kategori'); setKategoriMateriList(await response.json()); } catch (error: any) { toast({ title: "Error Kategori", description: error.message, variant: "destructive" }); } finally { setIsLoadingKategoriMateri(false); } };
  const fetchMateriAjarData = async () => { setIsLoadingMateriList(true); try { const response = await fetch('/api/kurikulum/materi-ajar'); if (!response.ok) throw new Error('Gagal mengambil materi'); setMateriList(await response.json()); } catch (error: any) { toast({ title: "Error Materi Ajar", description: error.message, variant: "destructive" }); } finally { setIsLoadingMateriList(false); } };
  const fetchMataPelajaranOptions = async () => { try { const response = await fetch('/api/mapel'); if (!response.ok) throw new Error('Gagal mengambil mapel'); setMataPelajaranOptions(await response.json()); } catch (error: any) { toast({ title: "Error Mapel", description: error.message, variant: "destructive" }); }};
  const fetchGuruOptions = async () => { try { const response = await fetch('/api/users?role=guru'); if (!response.ok) throw new Error('Gagal mengambil guru'); setGuruOptions(await response.json()); } catch (error: any) { toast({ title: "Error Guru", description: error.message, variant: "destructive" }); }};
  const fetchStrukturKurikulumData = async (tingkat: string, jurusan: string) => { setIsLoadingStruktur(true); try { const response = await fetch(`/api/kurikulum/struktur?tingkat=${encodeURIComponent(tingkat)}&jurusan=${encodeURIComponent(jurusan)}`); if (!response.ok) throw new Error(`Gagal mengambil struktur kurikulum`); setStrukturKurikulumData(prev => ({ ...prev, [`${tingkat}-${jurusan}`]: await response.json() })); } catch (error: any) { toast({ title: "Error Struktur Kurikulum", description: error.message, variant: "destructive" }); setStrukturKurikulumData(prev => ({ ...prev, [`${tingkat}-${jurusan}`]: [] })); } finally { setIsLoadingStruktur(false); } };
  const fetchSilabusData = async () => { setIsLoadingSilabus(true); try { const response = await fetch('/api/kurikulum/silabus'); if (!response.ok) throw new Error('Gagal mengambil silabus'); setSilabusList(await response.json()); } catch (error: any) { toast({ title: "Error Silabus", description: error.message, variant: "destructive" }); } finally { setIsLoadingSilabus(false); } };
  const fetchRppData = async () => { setIsLoadingRpp(true); try { const response = await fetch('/api/kurikulum/rpp'); if (!response.ok) throw new Error('Gagal mengambil RPP'); setRppList(await response.json()); } catch (error: any) { toast({ title: "Error RPP", description: error.message, variant: "destructive" }); } finally { setIsLoadingRpp(false); } };
  
  useEffect(() => { if (user && (user.role === 'admin' || user.role === 'superadmin')) { fetchSklData(); fetchCpData(); fetchKategoriMateriData(); fetchMateriAjarData(); fetchMataPelajaranOptions(); fetchGuruOptions(); fetchSilabusData(); fetchRppData(); } }, [user]);
