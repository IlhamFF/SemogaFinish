...
import { CalendarDays, Clock, UserCheck, AlertTriangle, PlusCircle, Edit, Search, Printer, Settings2, Building, Loader2, Trash2, GripVertical, Upload, Users, CalendarCheck, FileSymlink, ListFilter, Save, Eraser } from "lucide-react";
...
// Skema untuk satu entri jadwal pelajaran dalam dialog "Buat Jadwal Manual"
const jadwalPelajaranEntrySchema = z.object({
  id: z.string().uuid().optional(), // untuk entri yang sudah ada
...
export default function AdminJadwalPage() {
  ...
  const konfigurasiForm = useForm<KonfigurasiJadwalFormValues>({
    ...
    defaultValues: { slots: [], hariEfektif: hariEfektif },
  });
  const { fields: slotFields, append: appendSlot, remove: removeSlot } = useFieldArray({
    control: konfigurasiForm.control, name: "slots",
  });

  // State untuk "Buat Jadwal Manual per Kelas/Hari"
  ...
  const jadwalPelajaranEntryForm = useForm<JadwalPelajaranEntryFormValues>({
    resolver: zodResolver(jadwalPelajaranEntrySchema),
    defaultValues: { slotWaktuId: undefined, mapelId: undefined, guruId: undefined, ruanganId: undefined, catatan: "" },
  });
  ...
  useEffect(() => {
    if (user && (user.role === 'admin' || user.role === 'superadmin')) {
      fetchRuangan(); fetchSlotsWaktu(); fetchMapelOptions(); fetchGuruOptions();
    }
  }, [user, fetchRuangan, fetchSlotsWaktu, fetchMapelOptions, fetchGuruOptions]);
  ...
  useEffect(() => {
    if (isJadwalEntryFormOpen) {
      if (editingJadwalPelajaranEntry && 'slotWaktuId' in editingJadwalPelajaranEntry) { // Check if it's JadwalPelajaran or FormValues
        jadwalPelajaranEntryForm.reset({
            id: (editingJadwalPelajaranEntry as JadwalPelajaran).id, // if it's JadwalPelajaran
            slotWaktuId: editingJadwalPelajaranEntry.slotWaktuId,
            mapelId: editingJadwalPelajaranEntry.mapelId,
            guruId: editingJadwalPelajaranEntry.guruId,
            ruanganId: editingJadwalPelajaranEntry.ruanganId,
            catatan: editingJadwalPelajaranEntry.catatan || ""
        });
      } else if (editingJadwalPelajaranEntry) { // It's FormValues (for new entry from draft)
  ...
  const handlePlaceholderAction = (action: string) => toast({ title: "Fitur Dalam Pengembangan", description: `Fungsi "${action}" belum diimplementasikan.`});
  const handleSimulateAction = (action: string, description: string) => toast({ title: "Simulasi Berhasil", description: `Simulasi untuk "${action}" telah dijalankan. ${description}`});

  const handleRuanganFormSubmit = async (values: RuanganFormValues) => { /* ... (existing code) ... */ setIsRuanganSubmitting(true); const url = editingRuangan ? `/api/jadwal/ruangan/${editingRuangan.id}` : '/api/jadwal/ruangan'; const method = editingRuangan ? 'PUT' : 'POST'; const payload = editingRuangan ? { nama: values.nama, kapasitas: values.kapasitas, fasilitas: values.fasilitas } : values; try { const response = await fetch(url, { method: method, headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(payload),}); if (!response.ok) { const errorData = await response.json(); throw new Error(errorData.message || `Gagal ${editingRuangan ? 'memperbarui' : 'menambah'} ruangan.`); } toast({ title: "Berhasil!", description: `Ruangan ${values.nama} telah ${editingRuangan ? 'diperbarui' : 'ditambahkan'}.` }); setIsRuanganFormOpen(false); setEditingRuangan(null); fetchRuangan(); } catch (error: any) { toast({ title: "Error Ruangan", description: error.message, variant: "destructive" }); } finally { setIsRuanganSubmitting(false); }};
  const openDeleteRuanganDialog = (ruangan: Ruangan) => setRuanganToDelete(ruangan);
  const handleDeleteRuanganConfirm = async () => { /* ... (existing code) ... */ if (ruanganToDelete) { setIsRuanganSubmitting(true); try { const response = await fetch(`/api/jadwal/ruangan/${ruanganToDelete.id}`, { method: 'DELETE' }); if (!response.ok) { const errorData = await response.json(); throw new Error(errorData.message || 'Gagal menghapus ruangan.'); } toast({ title: "Dihapus!", description: `Ruangan ${ruanganToDelete.nama} telah dihapus.` }); fetchRuangan(); } catch (error: any) { toast({ title: "Error Hapus Ruangan", description: error.message, variant: "destructive" }); } finally { setIsRuanganSubmitting(false); setRuanganToDelete(null); } } };
  ...
  // Handlers for Jadwal Pelajaran Manual
  const handleOpenBuatJadwalManual = () => {
    setSelectedKelas(mockKelasList[0] || "");
    setSelectedHari(NAMA_HARI[0]);
    setIsBuatJadwalManualOpen(true);
  };

  const handleJadwalEntryFormSubmit = (values: JadwalPelajaranEntryFormValues) => {
    const newEntry: JadwalPelajaran = { // Create a display-ready object
      id: editingJadwalPelajaranEntry && 'id' in editingJadwalPelajaranEntry ? (editingJadwalPelajaranEntry as JadwalPelajaran).id : `draft-${Date.now()}`,
      hari: selectedHari,
      kelas: selectedKelas,
      ...values,
      slotWaktu: slotsWaktu.find(s => s.id === values.slotWaktuId),
      mapel: mapelOptions.find(m => m.id === values.mapelId),
      guru: guruOptions.find(g => g.id === values.guruId),
      ruangan: ruanganList.find(r => r.id === values.ruanganId),
    };
    
    if (editingJadwalPelajaranEntry && 'id' in editingJadwalPelajaranEntry) { // Editing existing
      setJadwalPelajaranList(prev => prev.map(j => j.id === newEntry.id ? newEntry : j));
    } else { // Adding new
      setJadwalPelajaranList(prev => [...prev, newEntry]);
    }
    setIsJadwalEntryFormOpen(false);
    setEditingJadwalPelajaranEntry(null);
  };

  const handleSaveJadwalKelasHari = async () => {
    if (!selectedKelas || !selectedHari || jadwalPelajaranList.length === 0) {
      toast({ title: "Info", description: "Pilih kelas, hari, dan tambahkan minimal satu jadwal.", variant: "default" });
      return;
    }
    setIsJadwalPelajaranSubmitting(true);
    // Prepare payload for batch API
    const payload = jadwalPelajaranList.map(j => ({
      hari: j.hari,
      kelas: j.kelas,
    ...
    try {
      // Clear existing schedule for this class & day first
      const deleteResponse = await fetch(`/api/jadwal/pelajaran/by-criteria?kelas=${encodeURIComponent(selectedKelas)}&hari=${encodeURIComponent(selectedHari)}`, {
        method: 'DELETE',
      });
      if (!deleteResponse.ok && deleteResponse.status !== 404) { // 404 is ok if no schedule existed
        const errorData = await deleteResponse.json();
        throw new Error(errorData.message || `Gagal membersihkan jadwal lama untuk ${selectedKelas} - ${selectedHari}.`);
      }

      // Then post the new batch
      const response = await fetch('/api/jadwal/pelajaran/batch', {
        method: 'POST',
      ...
      if (response.status === 207 && responseData.results) { // Multi-Status
        const successes = responseData.results.filter((r:any) => r.success).length;
        const failures = responseData.results.filter((r:any) => !r.success);
        if (failures.length > 0) {
          toast({ title: "Sebagian Jadwal Gagal Disimpan", description: `Berhasil: ${successes}, Gagal: ${failures.length}. Error: ${failures.map((f:any) => f.error).join("; ")}`, variant: "destructive", duration: 7000 });
        } else {
          toast({ title: "Berhasil!", description: `Jadwal untuk ${selectedKelas} - ${selectedHari} telah disimpan.` });
        }
        fetchJadwalPelajaran(selectedKelas, selectedHari); // Refresh
      } else if (!response.ok) {
        throw new Error(responseData.message || 'Gagal menyimpan jadwal pelajaran.');
      } else {
         toast({ title: "Berhasil!", description: `Jadwal untuk ${selectedKelas} - ${selectedHari} telah disimpan.` });
         fetchJadwalPelajaran(selectedKelas, selectedHari); // Refresh
      }
    } catch (error: any) {
      toast({ title: "Error Simpan Jadwal", description: error.message, variant: "destructive" });
    } finally {
      setIsJadwalPelajaranSubmitting(false);
    }
  };

  const handleEditJadwalEntry = (entry: JadwalPelajaran) => {
    setEditingJadwalPelajaranEntry(entry);
    jadwalPelajaranEntryForm.reset({
        id: entry.id,
        slotWaktuId: entry.slotWaktuId,
        mapelId: entry.mapelId,
        guruId: entry.guruId,
        ruanganId: entry.ruanganId,
        catatan: entry.catatan || ""
    });
    setIsJadwalEntryFormOpen(true);
  };
  
  const handleDeleteJadwalEntry = async (entryId: string) => {
    // If entryId is a draft ID, just remove from local state
    if (entryId.startsWith('draft-')) {
        setJadwalPelajaranList(prev => prev.filter(j => j.id !== entryId));
        toast({title: "Entri Dihapus dari Draf", description: "Entri jadwal telah dihapus dari daftar saat ini."});
        return;
    }
    // If it's a real ID, call API then refresh
    setIsJadwalPelajaranSubmitting(true);
    try {
        const response = await fetch(`/api/jadwal/pelajaran/${entryId}`, { method: 'DELETE' });
        if (!response.ok) {
            const errorData = await response.json();
            throw new Error(errorData.message || 'Gagal menghapus entri jadwal.');
        }
        toast({ title: "Entri Dihapus!", description: "Entri jadwal telah dihapus dari database."});
        if(selectedKelas && selectedHari) fetchJadwalPelajaran(selectedKelas, selectedHari);
    } catch (error: any) {
        toast({ title: "Error Hapus Entri", description: error.message, variant: "destructive" });
    } finally {
        setIsJadwalPelajaranSubmitting(false);
    }
  };

  const handleClearJadwalKelasHari = async () => {
    if (!selectedKelas || !selectedHari) return;
     setIsJadwalPelajaranSubmitting(true);
    try {
      const response = await fetch(`/api/jadwal/pelajaran/by-criteria?kelas=${encodeURIComponent(selectedKelas)}&hari=${encodeURIComponent(selectedHari)}`, {
        method: 'DELETE',
      });
       if (!response.ok && response.status !== 404) { // 404 is ok if no schedule existed
        const errorData = await response.json();
        throw new Error(errorData.message || `Gagal membersihkan jadwal untuk ${selectedKelas} - ${selectedHari}.`);
      }
      toast({ title: "Jadwal Dikosongkan", description: `Semua jadwal untuk ${selectedKelas} - ${selectedHari} telah dihapus.`});
      setJadwalPelajaranList([]); // Clear local state
    } catch (error: any) {
      toast({ title: "Error Kosongkan Jadwal", description: error.message, variant: "destructive" });
    } finally {
       setIsJadwalPelajaranSubmitting(false);
    }
  };
    ...
    <div className="space-y-6">
      ...
      <Card className="shadow-lg">
        <CardHeader>
          ...
          <CardDescription>Modul ini dirancang untuk memfasilitasi pembuatan, pengelolaan, dan publikasi jadwal pelajaran.</CardDescription>
        </CardHeader>
        <CardContent className="space-y-8">
          <Card>
            <CardHeader><CardTitle className="flex items-center text-xl"><Clock className="mr-3 h-5 w-5 text-primary" />Konfigurasi Dasar Jadwal</CardTitle></CardHeader>
            <CardContent className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
              ...
              <Button variant="outline" onClick={() => setIsImporJadwalOpen(true)} className="justify-start text-left h-auto py-3"><Upload className="mr-3 h-5 w-5" /><div><p className="font-semibold">Impor Jadwal</p><p className="text-xs text-muted-foreground">Unggah jadwal dari template (Simulasi).</p></div></Button>
            </CardContent>
          </Card>

          <Card>
            <CardHeader><CardTitle className="flex items-center text-xl"><UserCheck className="mr-3 h-5 w-5 text-primary" />Alokasi & Validasi</CardTitle></CardHeader>
            <CardContent className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
              <Button variant="outline" onClick={() => setIsKetersediaanGuruOpen(true)} className="justify-start text-left h-auto py-3"><Users className="mr-3 h-5 w-5" /><div><p className="font-semibold">Ketersediaan Guru</p><p className="text-xs text-muted-foreground">Lihat jadwal guru (Simulasi).</p></div></Button>
              <Button variant="outline" onClick={() => handleSimulateAction("Deteksi Konflik Jadwal", "Tidak ditemukan konflik berdasarkan data simulasi.")} className="justify-start text-left h-auto py-3"><AlertTriangle className="mr-3 h-5 w-5" /><div><p className="font-semibold">Deteksi Konflik</p><p className="text-xs text-muted-foreground">Periksa bentrok jadwal (Simulasi).</p></div></Button>
            </CardContent>
          </Card>
          
          <Card>
            <CardHeader><CardTitle className="flex items-center text-xl"><CalendarDays className="mr-3 h-5 w-5 text-primary" />Visualisasi & Publikasi</CardTitle></CardHeader>
            <CardContent className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
              <Button variant="outline" onClick={() => setIsTampilanJadwalOpen(true)} className="justify-start text-left h-auto py-3"><Search className="mr-3 h-5 w-5" /><div><p className="font-semibold">Tampilan Jadwal</p><p className="text-xs text-muted-foreground">Lihat jadwal per kelas/guru (Simulasi).</p></div></Button>
              <Button variant="outline" onClick={() => handleSimulateAction("Cetak Jadwal", "Jadwal telah diunduh (Simulasi).")} className="justify-start text-left h-auto py-3"><Printer className="mr-3 h-5 w-5" /><div><p className="font-semibold">Cetak Jadwal</p><p className="text-xs text-muted-foreground">Cetak jadwal (Simulasi).</p></div></Button>
              <Button variant="outline" onClick={() => handleSimulateAction("Publikasi Jadwal", "Jadwal telah dipublikasikan (Simulasi).")} className="justify-start text-left h-auto py-3"><FileSymlink className="mr-3 h-5 w-5" /> <div><p className="font-semibold">Publikasikan Jadwal</p><p className="text-xs text-muted-foreground">Umumkan jadwal (Simulasi).</p></div></Button>
            </CardContent>
          </Card>
        </CardContent>
      </Card>
      ...
      {/* Dialog Buat Jadwal Manual per Kelas/Hari */}
      <Dialog open={isBuatJadwalManualOpen} onOpenChange={setIsBuatJadwalManualOpen}>
        <DialogContent className="max-w-4xl max-h-[90vh] flex flex-col">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 py-4 border-b pb-6">
            <div>
              <Label htmlFor="select-kelas-jadwal">Pilih Kelas</Label>
              <Select value={selectedKelas} onValueChange={setSelectedKelas}>
                <SelectTrigger id="select-kelas-jadwal"><SelectValue placeholder="Pilih Kelas" /></SelectTrigger>
                <SelectContent>{mockKelasList.map(kls => <SelectItem key={kls} value={kls}>{kls}</SelectItem>)}</SelectContent>
              </Select>
            </div>
            <div>
              <Label htmlFor="select-hari-jadwal">Pilih Hari</Label>
              <Select value={selectedHari} onValueChange={setSelectedHari}>
                <SelectTrigger id="select-hari-jadwal"><SelectValue placeholder="Pilih Hari" /></SelectTrigger>
                <SelectContent>{hariEfektif.map(h => <SelectItem key={h} value={h}>{h}</SelectItem>)}</SelectContent>
              </Select>
            </div>
          </div>
            ...
            {!isLoadingJadwalPelajaran && selectedKelas && selectedHari && (
              <>
                <div className="flex justify-between items-center mb-4">
                    <h3 className="text-lg font-semibold">Jadwal untuk: {selectedKelas} - {selectedHari}</h3>
                    <Button size="sm" onClick={() => { setEditingJadwalPelajaranEntry(null); jadwalPelajaranEntryForm.reset(); setIsJadwalEntryFormOpen(true); }}><PlusCircle className="mr-2 h-4 w-4"/> Tambah Pelajaran</Button>
                </div>
                {jadwalPelajaranList.length > 0 ? (
                  <ScrollArea className="max-h-[calc(90vh-350px)]"> {/* Adjust max-h as needed */}
                    <Table>
                      <TableHeader><TableRow><TableHead>Slot Waktu</TableHead><TableHead>Mapel</TableHead><TableHead>Guru</TableHead><TableHead>Ruangan</TableHead><TableHead>Catatan</TableHead><TableHead className="text-right">Aksi</TableHead></TableRow></TableHeader>
                      <TableBody>
                        {jadwalPelajaranList.sort((a,b) => a.slotWaktu?.waktuMulai.localeCompare(b.slotWaktu?.waktuMulai || '') || 0).map(j => (
                          <TableRow key={j.id}>
                            <TableCell>{j.slotWaktu?.namaSlot} ({j.slotWaktu?.waktuMulai}-{j.slotWaktu?.waktuSelesai})</TableCell>
                            <TableCell>{j.mapel?.nama}</TableCell>
                            <TableCell>{j.guru?.fullName || j.guru?.name}</TableCell>
                            <TableCell>{j.ruangan?.nama}</TableCell>
                            <TableCell className="max-w-[150px] truncate" title={j.catatan || ""}>{j.catatan || "-"}</TableCell>
                            <TableCell className="text-right">
                              <Button variant="ghost" size="icon" onClick={() => handleEditJadwalEntry(j)} className="mr-1 h-7 w-7"><Edit className="h-4 w-4" /></Button>
                              <Button variant="ghost" size="icon" onClick={() => handleDeleteJadwalEntry(j.id)} className="text-destructive h-7 w-7"><Trash2 className="h-4 w-4" /></Button>
                            </TableCell>
                          </TableRow>
                        ))}
                      </TableBody>
                  ...
                  <p className="text-muted-foreground text-center py-6">Belum ada jadwal pelajaran untuk kelas dan hari ini.</p>
                )}
              </>
            )}
      ...
      {/* Dialog Form Tambah/Edit Entri Jadwal Pelajaran */}
      <Dialog open={isJadwalEntryFormOpen} onOpenChange={setIsJadwalEntryFormOpen}>
        <DialogContent className="sm:max-w-lg">
          <DialogHeader><DialogTitle>{editingJadwalPelajaranEntry ? "Edit Entri Jadwal" : "Tambah Entri Jadwal Baru"}</DialogTitle><DialogDescription>Untuk {selectedKelas} - {selectedHari}</DialogDescription></DialogHeader>
          <Form {...jadwalPelajaranEntryForm}>
            <form onSubmit={jadwalPelajaranEntryForm.handleSubmit(handleJadwalEntryFormSubmit)} className="space-y-4 py-4">
              <FormField control={jadwalPelajaranEntryForm.control} name="slotWaktuId" render={({ field }) => (<FormItem><FormLabel>Slot Waktu</FormLabel><Select onValueChange={field.onChange} value={field.value}><FormControl><SelectTrigger><SelectValue placeholder="Pilih slot waktu" /></SelectTrigger></FormControl><SelectContent>{slotsWaktu.sort((a,b)=> a.waktuMulai.localeCompare(b.waktuMulai)).map(s => <SelectItem key={s.id} value={s.id}>{s.namaSlot} ({s.waktuMulai}-{s.waktuSelesai})</SelectItem>)}</SelectContent></Select><FormMessage /></FormItem>)} />
              <FormField control={jadwalPelajaranEntryForm.control} name="mapelId" render={({ field }) => (<FormItem><FormLabel>Mata Pelajaran</FormLabel><Select onValueChange={field.onChange} value={field.value}><FormControl><SelectTrigger><SelectValue placeholder="Pilih mapel" /></SelectTrigger></FormControl><SelectContent>{mapelOptions.map(m => <SelectItem key={m.id} value={m.id}>{m.nama} ({m.kode})</SelectItem>)}</SelectContent></Select><FormMessage /></FormItem>)} />
              <FormField control={jadwalPelajaranEntryForm.control} name="guruId" render={({ field }) => (<FormItem><FormLabel>Guru Pengampu</FormLabel><Select onValueChange={field.onChange} value={field.value}><FormControl><SelectTrigger><SelectValue placeholder="Pilih guru" /></SelectTrigger></FormControl><SelectContent>{guruOptions.map(g => <SelectItem key={g.id} value={g.id}>{g.fullName || g.name} ({g.email})</SelectItem>)}</SelectContent></Select><FormMessage /></FormItem>)} />
              ...
              <FormField control={jadwalPelajaranEntryForm.control} name="catatan" render={({ field }) => (<FormItem><FormLabel>Catatan (Opsional)</FormLabel><FormControl><Textarea placeholder="Catatan tambahan..." {...field} value={field.value || ""} /></FormControl><FormMessage /></FormItem>)} />
              <DialogFooter><Button type="button" variant="outline" onClick={() => setIsJadwalEntryFormOpen(false)}>Batal</Button><Button type="submit">Simpan Entri</Button></DialogFooter>
            </form>
          </Form>
        </DialogContent>
      </Dialog>

      {/* Dialog-dialog lain yang masih placeholder (Impor, Ketersediaan Guru, Tampilan Jadwal) */}
      <Dialog open={isImporJadwalOpen} onOpenChange={setIsImporJadwalOpen}><DialogContent className="sm:max-w-md"><DialogHeader><DialogTitle>Impor Jadwal (Simulasi)</DialogTitle></DialogHeader><div className="py-4 space-y-4"><Input type="file" /><Button onClick={() => {handleSimulateAction("Impor File Jadwal", "File jadwal sedang diproses."); setIsImporJadwalOpen(false);}} className="w-full">Impor</Button></div><DialogFooter><Button variant="outline" onClick={() => setIsImporJadwalOpen(false)}>Tutup</Button></DialogFooter></DialogContent></Dialog>
      <Dialog open={isKetersediaanGuruOpen} onOpenChange={setIsKetersediaanGuruOpen}><DialogContent className="sm:max-w-lg"><DialogHeader><DialogTitle>Ketersediaan Guru (Simulasi)</DialogTitle></DialogHeader><div className="py-4 text-center">Data ketersediaan guru akan ditampilkan di sini.</div><DialogFooter><Button variant="outline" onClick={() => setIsKetersediaanGuruOpen(false)}>Tutup</Button></DialogFooter></DialogContent></Dialog>
      <Dialog open={isTampilanJadwalOpen} onOpenChange={setIsTampilanJadwalOpen}><DialogContent className="sm:max-w-xl"><DialogHeader><DialogTitle>Tampilan Jadwal (Simulasi)</DialogTitle></DialogHeader><div className="py-4 text-center">Tampilan jadwal per kelas/guru akan ditampilkan di sini.</div><DialogFooter><Button variant="outline" onClick={() => setIsTampilanJadwalOpen(false)}>Tutup</Button></DialogFooter></DialogContent></Dialog>
    </div>
  );
}
