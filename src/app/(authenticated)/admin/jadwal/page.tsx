    ...
    <div className="space-y-6">
      ...
      <Dialog open={isRuanganDialogOpen} onOpenChange={setIsRuanganDialogOpen}>
        <DialogContent className="sm:max-w-2xl">
          <DialogHeader>
            <DialogTitle>Manajemen Ruangan</DialogTitle>
            <DialogDescription>Lihat, tambah, edit, atau hapus data ruangan.</DialogDescription>
          </DialogHeader>
          <div className="py-4 space-y-4">
            <Button onClick={() => openRuanganForm()}><PlusCircle className="mr-2 h-4 w-4" /> Tambah Ruangan</Button>
            {isLoadingRuangan ? (<div className="flex justify-center items-center h-40"><Loader2 className="h-8 w-8 animate-spin text-primary" /></div>) : ruanganList.length > 0 ? (
          ...
          <Form {...konfigurasiForm}>
            <form onSubmit={konfigurasiForm.handleSubmit(handleKonfigurasiSubmit)} className="flex-grow flex flex-col overflow-y-hidden">
                <ScrollArea className="flex-grow -mx-6 px-6">
                    ...
                    <>
                      <div className="space-y-4">
                          <div>
                          <h3 className="text-lg font-medium mb-2">Slot Waktu</h3>
                          <div className="space-y-3">{slotFields.map((field, index) => (<Card key={field.id || `new-${index}`} className="p-3 relative"><div className="flex items-start gap-2"><div className="flex-grow grid grid-cols-1 sm:grid-cols-3 gap-3"><FormField control={konfigurasiForm.control} name={`slots.${index}.namaSlot`} render={({ field:ff }) => (<FormItem><FormLabel className="text-xs">Nama Slot</FormLabel><FormControl><Input placeholder="Jam ke-1" {...ff} /></FormControl><FormMessage className="text-xs"/></FormItem>)} /><FormField control={konfigurasiForm.control} name={`slots.${index}.waktuMulai`} render={({ field:ff }) => (<FormItem><FormLabel className="text-xs">Mulai</FormLabel><FormControl><Input type="time" {...ff} /></FormControl><FormMessage className="text-xs"/></FormItem>)} /><FormField control={konfigurasiForm.control} name={`slots.${index}.waktuSelesai`} render={({ field:ff }) => (<FormItem><FormLabel className="text-xs">Selesai</FormLabel><FormControl><Input type="time" {...ff} /></FormControl><FormMessage className="text-xs"/></FormItem>)} /></div><Button type="button" variant="ghost" size="icon" onClick={() => handleRemoveSlot(index)} className="text-destructive hover:text-destructive/80 absolute top-1 right-1 h-7 w-7"><Trash2 className="h-4 w-4" /></Button></div></Card>))}</div><Button type="button" variant="outline" size="sm" onClick={() => appendSlot({ namaSlot: "", waktuMulai: "00:00", waktuSelesai: "00:00", urutan: slotsWaktu.length + 1 })} className="mt-2"><PlusCircle className="mr-2 h-4 w-4"/> Tambah Slot</Button><FormMessage>{konfigurasiForm.formState.errors.slots?.message || konfigurasiForm.formState.errors.slots?.root?.message}</FormMessage>
                          </div>
                      </div>
                      <div>
                          <h3 className="text-lg font-medium mb-2">Hari Efektif</h3>
                          <FormField control={konfigurasiForm.control} name="hariEfektif" render={() => (<FormItem className="grid grid-cols-2 sm:grid-cols-3 gap-2">{NAMA_HARI.slice(0,6).map((hari) => (<FormField key={hari} control={konfigurasiForm.control} name="hariEfektif" render={({ field }) => (<FormItem className="flex flex-row items-start space-x-3 space-y-0 p-2 border rounded-md hover:bg-muted/50"><FormControl><Checkbox checked={field.value?.includes(hari)} onCheckedChange={(checked) => { return checked ? field.onChange([...field.value, hari]) : field.onChange(field.value?.filter(v => v !== hari))}}/></FormControl><FormLabel className="font-normal text-sm">{hari}</FormLabel></FormItem>)} />))} <FormMessage className="col-span-full">{konfigurasiForm.formState.errors.hariEfektif?.message}</FormMessage></FormItem>)} />
                      </div>
                    </>
                    )}
                    </div>
                ...
                <DialogFooter className="flex-shrink-0 border-t pt-4">
                  ...
                  <Button type="submit" disabled={isKonfigurasiSubmitting || isLoadingSlots}>{isKonfigurasiSubmitting && <Loader2 className="mr-2 h-4 w-4 animate-spin" />}Simpan</Button>
                </DialogFooter>
            </form>
          </Form>
        </DialogContent>
